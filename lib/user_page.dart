...
// Split response into parts, similar to Python code
final parts = response.split(' ');

if (response.startsWith('41 0D') && parts.length >= 3) {
try {
final speedHex = parts[2]; // Equivalent to response[11:13] in Python
...
try {
final rpmHex1 = parts[2]; // First byte
final rpmHex2 = parts[3]; // Second byte
...
try {
final throttleHex = parts[2]; // Equivalent to response[11:13] in Python
...
try {
// Send OBD commands as in Python code
_bluetoothConnection?.output.add(Uint8List.fromList('010D\r'.codeUnits)); // Speed
_bluetoothConnection?.output.add(Uint8List.fromList('010C\r'.codeUnits)); // RPM
_bluetoothConnection?.output.add(Uint8List.fromList('0111\r'.codeUnits)); // Throttle Position
} catch (e) {
